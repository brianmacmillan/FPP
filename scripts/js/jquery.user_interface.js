...
$.extend({bm_ui : {
	...
	determineRes : function(){
		...
		$.bm_ui.retina = (window.devicePixelRation>=2);
	},
	determineBrowser : function() {
		...
		$.bm_ui.browser_version = $.browser.version;
       ...
       //alert(" browser "+nua);
		...
		if (bMoz) $.bm_ui.browser="moz";
		if (bChrome) $.bm_ui.browser="chrome";  
		if (bIE)  $.bm_ui.browser="ie";
		if (bSafari) $.bm_ui.browser="safari";
		if (bOpera) $.bm_ui.browser="opera";
		if (bKonqueror) $.bm_ui.browser="konqueror";
		$.bm_ui.IPAD = navigator.userAgent.match(/iPad/i) != null;
	},
	determineDevice : function() {
		...
		if ($("#content.phone").length>0) $.bm_ui.PHONE=true;
	},
	...
	{ //used to highlight screen elements as part of the help
		try{
			$(pElement).css("borderColor",$.bm_ui.HIGHLIGHT);
			if (piZindex) {
	...
	{
		document.getElementById("message").innerHTML=psMessage;
	...
	reorient : function () {  
		...
		//$.bm_ui.setContentWidth($.bm_ui.current_tab);	
	 },
