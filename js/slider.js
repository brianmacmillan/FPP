  $(function() {
    $( "#slider-vertical" ).slider({
        ...
        change: function( event, ui ) {
            var sResult="";
            var iTemp = $("#slider-vertical").slider("option", "value");
            $("#slider-description").text("");

            $( "#slider-year" ).text( iTemp );
            ...
            if (iTemp==2016) iTemp=2002;
            if (iTemp == 1972){
              var s1972 = '<p>The U.S. Supreme Court dismisses Baker v. Nelson, one of three cases brought by same-sex couples. challenging the denial of marriage. </p>';
              iTemp = getIdFromState("Minnesota");
              $("#map-container").children("g").children("path").eq(iTemp).css("fill","red");
              $("#slider-description").html(s1974);              
            }
            ...
            if (iTemp == 1993) {
              ...
              $("#slider-description").html(s1993);                            
            }

            if (iTemp == 1996) {
              var s1996 = '<p>President Bill Clinton signs the so-called Defense of Marriage Act (DOMA) into law.</p>';
              s1996 += 'Following the world&#39s first-ever trial on the freedom to marry, led by co-counsel Dan Foley and Evan Wolfson, Judge Kevin Chang rules that the state did not have a legitimate reason for depriving same-sex couples of the freedom to marry.';
              iTemp=getIdFromState("Washington");
              $("#map-container").children("g").children("path").eq(iTemp).css("fill","red");

              iTemp=getIdFromState("Hawaii");
              $("#map-container").children("g").children("path").eq(iTemp).css("fill","lightgreen");
              $("#slider-description").html(s1996);                            
            }
            ...
            if (iTemp == 2003){
              ...
              $("#map-container").children("g").children("path").eq(iTemp).css("fill","lightgreen");
              $("#slider-description").html(s2003);
            }

            if (iTemp == 2004){
              var s2004 = '<p>Massachuessetts legislature passes law allowing same sex marriage.</p>'; 
              //s2004 += '<p>Vermont votes for same sex marriage</p>';
              s2004 += '<p>Anti-gay forces in eleven states, marshaled by Karl Rove, push through constitutional amendments to deny same-sex couples the freedom to marry.</p>';

              iTemp2 = getIdFromState("Massachusetts");
              $("#map-container").children("g").children("path").eq(iTemp2).css("fill","green");
              //iTemp2=getIdFromState("Vermont");
              //$("#map-container").children("g").children("path").eq(iTemp2).css("fill","green");
            ...
            if (iTemp == 2006){
              ...
              $("#slider-description").html(s2006);              
            }

            if (iTemp == 2007){
              var s2007 = '<p>Washington state Governor Christine Gregoire signs a domestic partnership bill into law. In the weeks that follow, Oregon Governor Ted Kulongski and New Hampshire Governor John Lynch also sign a domestic partnership law and a civil union law, respectively.</p>';
              ...
              $("#slider-description").html(s2007);              
            }

            if (iTemp == 2008){
              var s2008 = '<p>The California Supreme Court determines in "In Re: Marriage Cases," a case brought by Lambda Legal, the American Civil Liberties Union, and the National Center for Lesbian Rights, that a state statute excluding same-sex couples from marriage is unconstitutional.</p>';
              ...
              s2008 += '<p>Anti-gay forces push through Proposition 8, an anti-gay constitutional amendment that strips away same-sex couples’ freedom to marry and restricts marriage to different-sex couples. Similar amendments are passed in Florida and Arizona.</p>';
            ...
            if (iTemp == 2010){
              ...
              s2010 += '<p>Vermont pushes past civil union and embraces the freedom to marry when the state legislature overwhelmingly votes to override a veto from Governor Jim Douglas.</p>';
            ...
            if (iTemp == 2012){
              ...
              s2012 += '<p>The freedom to marry triumphs in all four states: Maine, Maryland, Minnesota, and Washington. Additionally, President Obama is reelected.</p>';
            ...
            if (iTemp == 2014){
              ...
              s2014 += '<p>The United States Supreme Court denies review in five different marriage cases, clearing the way for lower court rulings to stand and same-sex couples to finally have the freedom to marry in Utah, Oklahoma, Virginia, Indiana and Wisconsin.</p>';
              ...
              $("#slider-description").html(s2014);              
            }
            //$( "#slider-vertical" ).slider( "value",iTemp);

            //$( "#slider-year" ).val(iTemp);
        }
    });
    //$( "#slider-year" ).val( $( "#slider-vertical" ).slider( "value" ) );
  });
